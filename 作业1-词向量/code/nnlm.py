...
def main():
    ...
    with graph.as_default():
        ...
        def infer_output(input_data):
            ...
            # input_data 120*5
            emb_word_vector = tf.nn.embedding_lookup(embeddings,input_data)
            # emb_word_vector 120*5*50
            emb_word_vector = tf.reshape(emb_word_vector,[120,-1])
            # emb_word_vector 120*250
            # weight_h[:-1] 250*64
            hidden = tf.nn.tanh(tf.add(tf.matmul(emb_word_vector, weight_h[:-1]),weight_h[-1]))
            # 120*64 hidden layer
			# step 2: outputs = softmax(x * W + hidden * U + b)
            outputs = tf.add(tf.matmul(emb_word_vector,softmax_w),tf.matmul(hidden,softmax_u[:-1]))
            outputs = tf.add(outputs,softmax_u[-1])
